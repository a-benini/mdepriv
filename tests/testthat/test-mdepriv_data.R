test_that("mdepriv: argument data", {
  expect_equal(mdepriv(simul_data, c("y1", "y2", "y5")),
               mdepriv(as.matrix(simul_data), c("y1", "y2", "y5")))

  expect_error(mdepriv("simul_data", c("y1", "y2", "y5")),
               "The argument 'data' is neither of the class \"data.frame\" nor of the class \"matrix\".")

  expect_equal(names(mdepriv(simul_data, c("y1", "y2", "y5"), output = "data")),
               c(names(simul_data), "score_i"))

  expect_equal(names(mdepriv(simul_data, c("y1", "y2", "y5"), score_i_heading = "score_i_user_name", output = "data")),
  ...
  expect_error(mdepriv(model$data, model$items),
