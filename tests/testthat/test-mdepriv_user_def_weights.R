test_that("mdepriv: argument user_def_weights", {
  expect_equal(mdepriv(simul_data, c("y1", "y2"), user_def_weights = c(0.25, 0.75), output = "user_def_weights"),
               list('Dimension 1' = c(0.25, 0.75)))
  test_na <- mdepriv(simul_data, c("y1", "y2"), output = "user_def_weights")
  expect_equal(is.na(test_na), TRUE)

  expect_error(mdepriv(simul_data, c("y1", "y2"), user_def_weights = 1),
  ...
  expect_error(mdepriv(simul_data, c("y1", "y2"), user_def_weights = c(NA,0.5)),
  ...
  expect_error(mdepriv(simul_data, list(c("y1", "y2"), c("y3", "y4"), c("y5", "y6", "y7")),
  ...
  expect_error(mdepriv(simul_data, list('group A' = c("y1", "y2"), 'group B' = c("y3", "y4"), 'group C' = c("y5", "y6", "y7")),
