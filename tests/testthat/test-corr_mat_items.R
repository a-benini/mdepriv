test_that("corr_mat: argument items", {
  expect_error(corr_mat(simul_data, 1:4), "The argument 'items' is neither a vector nor a list of vectors constisting fully of character elements.")

  expect_error(corr_mat(simul_data, list('group A' = c("y1", "y2", "y3"), 'group A' = c("y4", "y5"))),
               "The argument 'items' contains duplicates among the labelled dimension names. If labelled, dimensions names must be unique."
               )

  expect_error(corr_mat(simul_data, list('group A' = c("y1", "y2", "y3"), c("y4", "y5"))),
  ...
  expect_equal(as.vector(corr_mat(simul_data, "y1")), 1)

  expect_error(corr_mat(simul_data, c("y1", "y2", "y2")),
               "The argument 'items' contains repeatedly the following variable: \"y2\".")

  expect_error(corr_mat(simul_data, c("y1", "y1", "y2", "y2")),
               "The argument 'items' contains repeatedly the following variables: \"y1\" and \"y2\".")

  expect_error(corr_mat(simul_data, c("y1", "y2", "X")),
  ...
  expect_error(corr_mat(simul_data, c("y1", "Z", "X")),
  ...
  expect_error(corr_mat(simul_data, c("y1", "y2", "id")))
  expect_error(corr_mat(simul_data, c("y1", "y2", "id", "sampl_weights")))

  simul_data_2 <- simul_data
  simul_data_2[1, c("y1", "y2")] <- NA
  expect_error(corr_mat(simul_data_2, c("y1", "y3", "y4")),
               "The argument 'items' contains the following variable with NA: \"y1\". The argument 'items' does not allow any NA-values."
               )
  expect_error(corr_mat(simul_data_2, c("y1", "y2", "y3")),
  ...
  expect_error(corr_mat(simul_data_2, c("y1", "y2", "A")),
  ...
  expect_error(corr_mat(simul_data_2, c("y1", "y2", "A", "B")),
