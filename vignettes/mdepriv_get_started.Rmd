...
class <- c("single character string", "single numeric value", rep("data.frame", 3),
           "numeric vector", "single numeric value", "data.frame", "list",
           rep("single character string", 3), "single numeric value",
           "list or single NA-value", "single character string")
...
rbind(without_s_w, with_s_w) %>%
  ...
  theme_bw() +
  ggtitle('method vs. aggregate deprivation level')
```

Lets investigate the differences of the `method`s not just based on the single figure `aggregate_deprivation_level`, but on a comparison of `summary_by_item`-tables.
...
Different `method`s do matter as far as their result, but this should not be confused with the default argument settings of `mdepriv`:
...
plot_corr_val_rhoH <- function(data      = simul_data,
                               ...
                               sampling_weights = NA,
                               corr_type = c("mixed", "pearson", "polychoric"),
                               min_y = -0.2
  ){
  corr_type <- match.arg(corr_type)
  corr_val <- corr_mat(data = data, items = items, sampling_weights = sampling_weights,  corr_type = corr_type) %>% sort() %>% unique()
...
tibble(wa, wb, summary_by_item) %>% # gather (variated) arguments & output in a tibble (i.e. data.frame)
  ...
  theme_bw() +
  ggtitle('Item Weight from User Defined Double Weighting Schemes')
...
```
<a name="reuse_data"></a>
Thus the scores are twofold accessible:
```{r}
items_sel <- c("y1", "y2", "y3", "y4", "y5", "y6", "y7") # a selection of items
model <- mdepriv(simul_data, items_sel, output = "all") # method (default) = "cz"
all.equal(model$score_i, model$data$score_i)
```
The score column heading effects the ability of the `output` element `"data"` being used as input `data` in another model:
```{r error=TRUE, purl = FALSE}
...
# all correlation type are "polychoric"
```
